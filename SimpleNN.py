...
def train(model, criterion, optimizer, filepaths, labels, device, epochs, image_dimensions, augment):
...
=======
  ...
  with torch.no_grad(): 
    for i in range(0, total_samples, batch_size):
      batch_inputs = load_images(filepaths[i : i + batch_size], device, dimensions=image_dimensions, augmentation="test")
      batch_labels = labels[i : i + batch_size]

      # Forward pass: compute predicted outputs
      outputs = model(batch_inputs)
>>>>>>> dion-architecture-tests
